...
- Added the 0.7 public-network provider core as groundwork for the upcoming IP/geolocation option: the current `country.is` behavior remains the production default for now, while validated adapters for Cloudflare trace, ipwho.is, FreeIPAPI, and GeoJS plus an IP-only Amazon check endpoint can be exercised independently. Real PIA virtual-location comparison established FreeIPAPI, GeoJS, and ipwho.is as the maintained online candidates; `country.is` and Cloudflare are not planned as selectable country providers.
