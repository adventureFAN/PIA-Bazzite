...
- Added the 0.7 Stage 3D server-marker polish candidate: the main server selector replaces the verbose virtual-location and streaming text suffixes with neutral monochrome `◇` and `▶` markers. Hover QuickInfo explains each marker in DE/EN, while existing favorite/unavailable tooltips remain part of the same row tooltip. Verbose region wording outside the main selector is intentionally unchanged.
