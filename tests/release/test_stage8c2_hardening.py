...
class Stage8C2StaticHardeningTests(unittest.TestCase):
    ...
    def test_ui_polish_uses_real_popup_cap_tools_menu_and_region_tray_label(self) -> None:
        source = (ROOT / "pia_bazzite/gui.py").read_text(encoding="utf-8")
        ...
        kill_pos = source.index('self.options_menu.addAction(self.kill_switch_action)')
        credentials_pos = source.index('self.options_menu.addAction(self.credentials_action)', kill_pos)
        log_pos = source.index('self.options_menu.addAction(self.live_log_action)', credentials_pos)
        dialog_pos = source.index('self.options_menu.addAction(self.options_dialog_action)', log_pos)
        self.assertLess(kill_pos, credentials_pos)
        self.assertLess(credentials_pos, log_pos)
        self.assertLess(log_pos, dialog_pos)
        self.assertIn('self.options_menu.setTitle(tr("menu.tools"))', source)
