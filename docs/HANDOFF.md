...
0.7 Stage 5A addresses that stale presentation only: it adds a separate physical-underlay watcher so the GUI can distinguish “WireGuard profile administratively active” from “a physical NetworkManager uplink is currently available”. It does **not** replace the audited VPN/Kill-Switch state machine, add a handshake-age/HTTP watchdog, or weaken the existing firewall/recovery contracts.
...
- Stage 3A replaced the former visible `Options` / `Optionen` menu with `Tools` in English and initially `Funktionen` in German so the menu can contain an `Options…` / `Optionen …` dialog entry without the awkward `Options → Options…` duplication. Stage 6A later resolves the purely cosmetic German label to the more conventional desktop term `Extras` after KDE-HIG review; this does not reopen Stage 3A behavior. The internal `options_menu` object name is not a public UX contract.
...
- `Re-enter credentials…`, `Show Live Log`, and the Kill Switch toggle deliberately remain direct Tools/Extras actions. Credential re-entry is a command rather than a preference; Live Log is a frequent diagnostic/view toggle and keeps `Ctrl+L`; the Kill Switch is an immediate security state change that may require privileged verification and therefore must not be hidden behind a generic modal settings transaction.
...
- Focused static regression coverage lives in `tests/ui/test_options_dialog_stage3a.py`; `tools/0.7-stage3a-options-window-self-test.sh` runs it together with the existing release self-test without importing/running the real Qt GUI. The focused Stage 3A gate and inherited release self-test passed. Real Bazzite verification is complete: the dialog keeps its fixed size; all selector fields use the accepted common alignment/width; Cancel leaves all preferences unchanged; language and System/Light/Dark appearance changes apply correctly; all active-VPN quit-behavior choices retain the established behavior; tray visibility can be disabled/re-enabled; and saved choices persist across reopening/restart. The German `Funktionen` label was accepted for Stage 3A; Stage 6A later performs the explicitly separated cosmetic rename to `Extras` without changing the verified dialog behavior. Treat Stage 3A as verified.
...
- Focused coverage is `tests/ui/test_autostart_tray_stage4c_07.py`; `tools/0.7-stage4c-autostart-tray-polish-self-test.sh` also reruns Stage 4B, Stage 4A, Stage 2C, Stage 3A, and single-instance/static regression coverage. Real Bazzite verification confirms the Plasma top-level tray icon rendering, a dry `--autostart` launch, a full reboot/login XDG-autostart path with Auto-Connect, hidden-in-tray startup when the tray is enabled, and visible-main-window fallback when the tray is disabled. The Auto-Connect Options popup deliberately opens at row zero every time, matching the main server selector and ensuring `Off` is immediately visible even when another target is selected. Final selector polish groups the special modes first (`Off`, `Last selected location`, then Fastest), gives Off/Last neutral icons, keeps gold stars on favorite rows, removes only the redundant disabled `Favorites` heading, and then shows the ordinary ping-sorted region list. Stage 6A later makes the Fastest icon neutral as well. The final selector presentation was visually confirmed on real Bazzite after the popup-top fix: `Off` is always immediately visible, Off/Last/Fastest are clearly distinguished from ordinary regions, favorites retain their gold stars without a redundant `Favorites` heading, and the normal region list follows cleanly below. Treat Stage 4C as verified and frozen; no further functional autostart/tray behavior changes are planned.

**Stage 5A verified — physical-network state detection (2026-08-13):**

- Real Bazzite baseline diagnosis: with a protected PIA WireGuard session active, disabling Wi-Fi for more than one minute left the 0.7 GUI Green because `network_manager.is_connected()` correctly continued to see the administratively active WireGuard profile. In parallel, `nmcli monitor` reported the Wi-Fi device deactivation, loss of the primary connection, and later reconnection immediately. This confirms a missing underlay watcher rather than a slow NetworkManager notification path.
- The existing 0.6 security result remains authoritative: the full Session Kill Switch stayed active and public IPv4/IPv6 remained blocked during physical-network loss. Stage 5A is therefore a UI/runtime-reconciliation improvement, **not** a fix for a demonstrated fail-open.
- The verified implementation starts one user-owned `nmcli monitor` process as an event trigger only. Its localized human-readable output is deliberately never parsed. Every event is debounced and followed by an authoritative `nmcli -t -f GENERAL.DEVICE,GENERAL.TYPE,GENERAL.STATE device show` query; only NetworkManager numeric device state `100` on an uplink-capable device counts as an available underlay. A 3-second fallback query runs alongside the existing VPN status poll if the monitor process stops. Global NetworkManager “connectivity” is intentionally not used because a correctly enforced Kill Switch can make KDE/NetworkManager report Limited connectivity while the physical uplink is healthy.
- When NetworkManager still reports the PIA WireGuard profile active but the physical underlay disappears, the audited Kill-Switch runtime mode is preserved and only presentation is overlaid. Verified Session Kill Switch protection becomes **Orange** with explicit “Network unavailable / VPN waiting for network / Kill Switch active” copy; normal VPN mode becomes neutral Grey rather than falsely staying Blue. Existing Red protection errors always keep priority. Public-IP/country/DNS fields are suppressed while the underlay is absent.
- Server-changing/connect surfaces that require a usable physical path are disabled while the underlay is unavailable, but an already-active VPN can still be deliberately disconnected and protection recheck/reset paths remain available as appropriate. The tray top status changes to `Network unavailable`, and the tray/main status recover when the physical path returns. A quiet public-network refresh is scheduled after restoration if NetworkManager still reports the VPN active.
- Stage 5A deliberately adds **no new automatic reconnect algorithm**, no delayed Auto-Connect after a failed offline startup, no automatic server-list refresh, and no endpoint/handshake watchdog. Existing protected reconnect remains responsible only when NetworkManager actually reports the PIA VPN itself down. After real host testing, Stage 5B was consciously omitted: the previously audited Wi-Fi/LAN/NetworkManager recovery behavior is already sufficient, and no extra reconnect machinery should be added without a concrete failure case. Trusted-network policy remains a separate 0.8 feature.
- Focused deterministic coverage: `tests/connection/test_physical_network_state_stage5a_07.py` validates numeric device-state parsing and virtual-device exclusion; `tests/ui/test_network_state_detection_stage5a_07.py` validates the event-trigger/fallback architecture, Orange/Grey presentation overlay, Red-error priority, tray/control gating, clean monitor shutdown and DE/EN copy. `tools/0.7-stage5a-network-state-detection-self-test.sh` also reruns Stage 4B/4C regressions and prints the current real NetworkManager underlay probe. Final local source gate: **34/34 focused + Stage 4B/4C regression tests PASS** in the development container.
...
- The compact main window without Live Log is reduced by 50 px from 740 to **690 px** width. The expanded Live-Log width remains **760 px** so diagnostic text is not squeezed.
- The compact `●` marker explains the non-obvious virtual-location concept in QuickInfo: `Virtual location / Virtueller Standort` plus one short sentence that the physical server is in another country. Stage 6A initially removed the `▶` streaming QuickInfo too aggressively; real Bazzite review caught that the marker still needs its one-line legend. Stage 6B restores only `Streaming-optimized location / Streaming-optimierter Standort` with no redundant explanatory sentence.
- Main-server filtering is integrated directly into the existing search field through one small filter action/menu with four mutually exclusive modes: **All, Normal, Virtual, Streaming-optimized**. Normal means neither virtual nor streaming. An active type filter combines with text search; the dynamic Fastest pseudo-row is shown only in the unfiltered/no-query view so it can never bypass the visible filter. Missing favorite snapshots are filtered by their safe stored `geo`/name metadata too.
...
- Security/error notifications are intentionally complementary to Plasma/NetworkManager rather than duplicating normal VPN connect/disconnect popups. PIA Bazzite only emits its own notification for important background events such as verified Kill-Switch blocking/error transitions or an Auto-Connect startup failure, and only when Main is not visible and the tray can display messages.
- The old one-per-session `Window minimized to the system tray` notification is removed completely; the visible tray icon and explicit tray `Quit` action already make the behavior clear, and the repeated hint was a real Bazzite/KDE annoyance.
- `Confirm server switches` controls only the existing confirmation dialog. When disabled, the selected server still uses the exact existing protected/unprotected switch path; this option must never bypass Kill-Switch safety or introduce a second switch implementation.
- `Show public IP and location` controls both presentation and the associated external lookup. When disabled, the public-IP/country rows are hidden, the manual public-info action is disabled, and `refresh_public_info()` returns before contacting GeoJS/FreeIPAPI/ipwho.is. The provider selector is disabled in Options while this display/lookup is off. IPv6-guard and DNS protection rows remain visible because they are local safety state, not public geolocation.
...
- Historical 0.6.0 menu-order note: `Kill Switch verwenden` followed the `Beim Beenden mit aktivem VPN` submenu. In 0.7 Stage 3A those ordinary preference submenus move into the Options dialog, while the direct toggle remains in Tools/Extras and is renamed `Session Kill Switch verwenden`.
...
- Stage 5A physical-network detection must remain an **overlay around** the existing audited VPN/Kill-Switch state, not a replacement for it. `nmcli monitor` output is only a wake-up signal; never parse its localized prose for security decisions. NetworkManager numeric device state is the source for physical-underlay availability, and existing Red protection errors must always outrank the Orange/Grey network-unavailable presentation.

### Stage 6 UI freeze (2026-08-13)
- **Stage 6A UI/search/filter polish is VERIFIED and FROZEN on real Bazzite.** Final host-approved behavior includes the 690 px compact main window, unchanged larger Live-Log state, `Extras` in DE / `Tools` in EN, compact in-line location filtering, combined text search + type filtering, semantic search terms for normal/virtual/streaming locations, stable neutral non-favorite stars, monochrome action/mode icons, gold only for actual favorite state, and the concise marker tooltips (`● Virtueller Standort` plus its one-line physical-location explanation; `▶ Streaming-optimierter Standort` without redundant explanation).
- **Stage 6B tabbed Options redesign and KDE polish are VERIFIED and FROZEN on real Bazzite.** Final tabs are `Allgemein`, `Verbindung`, and `Netzwerk && Datenschutz` (rendered as `Netzwerk & Datenschutz`). Duplicate inner tab headings are removed. Options use a robust two-column grid with full-width checkbox rows, avoiding Breeze/Plasma label overlap or truncation while keeping combo boxes aligned. The noisy minimize-to-tray notification remains removed. Tray tooltip copy uses a colon in the first line (`PIA Bazzite: …`); Qt's `QSystemTrayIcon` does not expose KDE StatusNotifierItem title/subtitle styling separately, so no custom tray backend is introduced merely for mixed font sizes.
- Stage 6 host visual review is complete and accepted as “alles wie aus einem Guss / optisch tiptop”. Automated Stage 6B focused/regression coverage passed **95/95**, `self_test.py` passed, and the authoritative unprivileged release gate passed. No more Stage 6 visual changes are planned before the 0.7 release-regression phase.
- Planned release validation still includes Plasma UI scaling checks at **125% and 150%**; this remains a Stage 7 release test rather than a reason to reopen Stage 6.

### Stage 7B.3 Light-theme icon contrast verified (2026-08-13)
- Release UI matrix found one minor real-Plasma issue: with PIA Bazzite forced to Light mode while the desktop/icon theme remained dark, the Auto-Connect `Last selected location` theme icon could stay white and become low-contrast.
- Candidate fix deliberately removes desktop-theme pixel dependence for the Auto-Connect special modes. `Off`, `Last selected`, and neutral `Fastest` are rendered from the current combo palette; explicit Normal/Selected/Disabled pixmaps use `Text`, `HighlightedText`, and disabled `Text` roles. Actual favorite stars remain gold.
...
- **7H PASS:** fresh-start defaults and first-run behavior were validated. A fully isolated `XDG_CONFIG_HOME` was found unsuitable for testing the `System` theme because it also hides KDE's theme configuration; an app-only fresh-profile check correctly followed the real Plasma Dark setting. During the later Gear Lever replacement test, the previous 0.6.0 settings/credentials were not retained. Because the fresh-profile test had manipulated the app's configuration immediately beforehand, this single observation does not isolate a reproducible application migration defect. Cross-version 0.6.0→0.7.0 settings migration is therefore **not claimed as verified**. Persistence of newly saved 0.7.0 settings across normal AppImage restarts is verified.
- **7I PASS:** release metadata/version/release notes and all authoritative packaging gates passed. The committed 0.7.0 release-mode AppImage was built successfully, its SHA-256 verified, and the artifact reported `PIA Bazzite 0.7.0`. The exact AppImage was integrated through Gear Lever and passed the real-Bazzite runtime smoke test for ordinary VPN, Session Kill Switch, clean disconnect, settings persistence, and login autostart. Remaining release work is repository/tag publication only. Public stable remains 0.6.0 until tag `v0.7.0` and the GitHub release are published successfully.
