...
**Current development baseline:** **0.7.0 development — Stage 1 verified; Stage 2 server favorites fully verified; Stage 3A Options-window foundation verified; Stage 3B public-network provider core verified**
...
- **0.7.x current sequence:** Stage 3A Options-window foundation and Stage 3B provider-core/real-location comparison are complete and verified. Next build the small provider choice on that foundation, then implement and verify the Automatic/local-IPinfo-Lite path before changing the production default away from `country.is`. Auto-Connect and improved network-change handling remain later candidates. Server Favorites are complete and verified.
