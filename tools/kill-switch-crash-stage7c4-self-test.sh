...
REPORT="$REPORT_DIR/pia-kill-switch-crash-stage7c4-self-test.txt"
mkdir -p "$REPORT_DIR"
exec > >(tee "$REPORT") 2>&1

printf 'PIA Bazzite stage-7C.4 peer-pipe retained-session proof gate self-test\n'
...
printf '%s\n' '--- Stage-5/6 connection and Stage-7C.4 corrected peer-pipe integration tests ---'
