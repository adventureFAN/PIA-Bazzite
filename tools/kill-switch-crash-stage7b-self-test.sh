...
REPORT="$REPORT_DIR/pia-kill-switch-crash-stage7b-self-test.txt"
mkdir -p "$REPORT_DIR"
exec > >(tee "$REPORT") 2>&1

printf 'PIA Bazzite stage-7B GUI crash-record integration and SIGKILL gate self-test\n'
...
printf '%s\n' '--- Stage-5/6 connection and Stage-7B crash-record integration tests ---'
...
printf '\nALL STAGE-7B GUI CRASH-PERSISTENCE GATE SELF-TESTS PASSED\n'
