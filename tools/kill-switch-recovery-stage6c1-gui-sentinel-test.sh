...
REPORT="$REPORT_DIR/pia-kill-switch-recovery-stage6c1-gui-sentinel-test.txt"
RESET_UNIT="pia-bazzite-stage6c1-gui-safety-reset"
RESET_DELAY="15min"
TABLE="pia_bazzite_killswitch"
mkdir -p "$REPORT_DIR"
exec > >(tee "$REPORT") 2>&1

printf 'PIA Bazzite stage-6C.1 real GUI test with independent physical-path sentinel\n'
...
if sudo systemctl is-active --quiet "${RESET_UNIT}.timer"; then
  ...
  fail "The independent Stage-6C.1 safety-reset timer is not active."
fi
...
case "$DRIVER_STATUS" in
  0)
    ...
    printf '\nALL STAGE-6C.1 REAL GUI HOST TESTS PASSED\n'
    printf 'Report: %s\n' "$REPORT"
    ;;
  20)
    cancel_reset_timer
    printf '\nStage 6C.1 stopped before a production firewall lock was observed.\n'
    printf 'Report: %s\n' "$REPORT"
    exit "$DRIVER_STATUS"
    ;;
  *)
