...
REPORT="$REPORT_DIR/pia-kill-switch-crash-stage7c2-host-test.txt"
RESET_UNIT="pia-bazzite-stage7c2-takeover-safety-reset"
RESET_DELAY="15min"
TABLE="pia_bazzite_killswitch"
mkdir -p "$REPORT_DIR"
exec > >(tee "$REPORT") 2>&1

printf 'PIA Bazzite stage-7C.2 privileged retained-session proof test\n'
printf 'Generated: %s\n' "$(date --iso-8601=seconds)"
printf 'This test repeats the real crash takeover after the Stage-7C.1 ordering fix and corrects the process proof for root-owned helpers.\n'
printf 'A rotated recovery record is accepted only while the root-visible restricted helper remains\n'
printf 'an exact descendant of the restarted GUI before normal disconnect.\n\n'
...
if sudo "$NFT_BIN" list table inet "$TABLE" >/dev/null 2>&1; then
  fail "A previous production firewall lock exists. Run the Stage-7C.2 emergency reset first."
fi
...
if sudo systemctl is-active --quiet "${RESET_UNIT}.timer"; then
  ...
  fail "The independent Stage-7C.2 safety-reset timer is not active."
fi

printf '\n%s\n' '--- Real GUI crash and automatic takeover observation ---'
set +e
env -u PYTHONPATH "$ROOT/.venv/bin/python" \
  "$ROOT/tools/pia-bazzite-stage7c-takeover-driver.py"
DRIVER_STATUS=$?
set -e
...
case "$DRIVER_STATUS" in
  0)
    ...
    printf '\nALL STAGE-7C.2 PRIVILEGED RETAINED-SESSION PROOF HOST TESTS PASSED\n'
    printf 'Report: %s\n' "$REPORT"
    ;;
  20)
    cancel_reset_timer
    printf '\nStage 7C.2 stopped before a production firewall lock was observed.\n'
    printf 'Report: %s\n' "$REPORT"
    exit "$DRIVER_STATUS"
    ;;
  *)
    ...
    printf '  cd %q && bash tools/kill-switch-crash-stage7c2-emergency-reset.sh\n' "$ROOT"
    printf 'Report: %s\n' "$REPORT"
    exit "$DRIVER_STATUS"
    ;;
esac
