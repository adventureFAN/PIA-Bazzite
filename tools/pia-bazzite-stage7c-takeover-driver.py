...
for entry in Path("/proc").iterdir():
    ...
    try:
        ...
        continue
    if needle in cmdline and descends_from(pid):
        print(pid)
        raise SystemExit(0)
raise SystemExit(1)
"""
...
def _unprivileged_session_descendant(app_pid: int) -> int | None:
    """Best-effort process-tree check without elevated /proc access."""
    ...
    for entry in Path("/proc").iterdir():
        ...
        try:
            ...
            # A root-owned, non-dumpable helper may deliberately hide cmdline
            # from the desktop user after pkexec has completed.
            continue
        if SESSION_HELPER not in cmdline:
            continue
        if _descends_from(pid, app_pid):
            return pid
    return None


def _privileged_session_descendant(app_pid: int) -> int | None:
    """Verify the root helper process through the already-authorized host-test boundary."""
    ...
    if completed.returncode != 0:
        ...
        raise TakeoverHostTestError(
            "The privileged restricted-session process probe failed"
            + (f": {detail}" if detail else ".")
        )
    output = completed.stdout.strip()
    if not output.isdecimal():
        raise TakeoverHostTestError(
            "The privileged restricted-session process probe returned an invalid PID."
        )
    return int(output)


def _session_descendant(app_pid: int, *, privileged: bool) -> int | None:
    session_pid = _unprivileged_session_descendant(app_pid)
    if session_pid is not None or not privileged:
        return session_pid
    return _privileged_session_descendant(app_pid)
...
def _wait_for_clean_startup_reconciliation(
    ...
    *,
    ...
    while time.monotonic() < deadline:
        ...
        session_pid = _session_descendant(app_process.pid, privileged=False)
        if session_pid is not None:
            if session_pid != stable_pid:
                stable_pid = session_pid
                stable_since = time.monotonic()
            elif stable_since is not None and time.monotonic() - stable_since >= 1.0:
                print(
                    "PASS    The GUI automatically opened and retained the restricted helper session on a clean startup.",
                    flush=True,
                )
                print(
    ...
    *,
    ...
    while time.monotonic() < deadline:
        ...
        if adopted and rotated and same_payload:
            session_pid = _session_descendant(app_process.pid, privileged=True)
            if session_pid is None:
                ...
                if rotated_without_session_since is None:
                    ...
                    raise TakeoverHostTestError(
                        "The restarted GUI rotated the recovery record but did not retain "
                        "the restricted helper session for 10 seconds."
                    )
            else:
                rotated_without_session_since = None
                if record != stable_record or session_pid != stable_session_pid:
                    stable_record = record
                    stable_session_pid = session_pid
                    stable_since = time.monotonic()
                    ...
                    print(
                        "PASS    The restarted GUI retained the authenticated helper session before rotating the recovery record.",
                        flush=True,
                    )
                    print(
                        f"PASS    Root-visible restricted helper PID {session_pid} remained a descendant of restarted GUI PID {app_process.pid}.",
                        flush=True,
                    )
                    print(
