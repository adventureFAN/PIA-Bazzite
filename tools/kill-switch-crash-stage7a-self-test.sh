...
REPORT="$REPORT_DIR/pia-kill-switch-crash-stage7a-self-test.txt"
mkdir -p "$REPORT_DIR"
exec > >(tee "$REPORT") 2>&1

printf 'PIA Bazzite stage-7A crash-recovery state and adoption-boundary self-test\n'
...
printf '%s\n' '--- Stage-5/6 connection and Stage-7A crash-recovery tests ---'
...
printf '\nALL STAGE-7A CRASH-RECOVERY STATE SELF-TESTS PASSED\n'
printf 'No host firewall, VPN connection, GUI session, or crash-recovery record was changed by this self-test.\n'
printf 'Report: %s\n' "$REPORT"
