...
REPORT="$REPORT_DIR/pia-kill-switch-crash-stage7c-self-test.txt"
mkdir -p "$REPORT_DIR"
exec > >(tee "$REPORT") 2>&1

printf 'PIA Bazzite stage-7C automatic GUI crash-takeover gate self-test\n'
...
printf '%s\n' '--- Stage-5/6 connection and Stage-7C crash-takeover integration tests ---'
...
printf '\nALL STAGE-7C AUTOMATIC GUI CRASH-TAKEOVER GATE SELF-TESTS PASSED\n'
