...
REPORT="$REPORT_DIR/pia-kill-switch-connection-stage5b-self-test.txt"
mkdir -p "$REPORT_DIR"
exec > >(tee "$REPORT") 2>&1

printf 'PIA Bazzite stage-5B production-boundary and intentional-disconnect self-test\n'
...
printf '%s\n' '--- Stage-5 connection and production-boundary tests ---'
