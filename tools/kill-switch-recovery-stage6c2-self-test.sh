...
REPORT="$REPORT_DIR/pia-kill-switch-recovery-stage6c2-self-test.txt"
mkdir -p "$REPORT_DIR"
exec > >(tee "$REPORT") 2>&1

printf 'PIA Bazzite stage-6C.2 emergency-reset recovery and single-instance gate self-test\n'
printf 'Generated: %s\n' "$(date --iso-8601=seconds)"
printf 'This test does not use sudo, pkexec, networking, NetworkManager, nftables, the GUI, or the leak sentinel.\n\n'
...
mapfile -t python_files < <(
  ...
  printf '%s\n' \
    ...
    "$ROOT/tools/pia-bazzite-stage6b-leak-sentinel.py" \
...
for script in \
  ...
  "$ROOT/tools/kill-switch-recovery-stage6a-self-test.sh" \
  "$ROOT/tools/kill-switch-recovery-stage6b-self-test.sh" \
  "$ROOT/tools/kill-switch-recovery-stage6b-host-test.sh" \
  "$ROOT/tools/kill-switch-recovery-stage6b-emergency-reset.sh" \
  "$ROOT/tools/kill-switch-recovery-stage6c-self-test.sh" \
  "$ROOT/tools/kill-switch-recovery-stage6c1-self-test.sh" \
  "$ROOT/tools/kill-switch-recovery-stage6c2-self-test.sh" \
  "$ROOT/tools/kill-switch-recovery-stage6c1-gui-sentinel-test.sh" \
  "$ROOT/tools/kill-switch-recovery-stage6c2-gui-sentinel-test.sh" \
  "$ROOT/tools/pia-bazzite-stage2-helper-installer.sh"
do
  bash -n "$script"
done
printf 'PASS  stage-5/6 shell and installer scripts parse\n\n'

printf '%s\n' '--- Stage-5 connection and Stage-6 recovery/GUI tests ---'
...
printf '\nALL STAGE-6C.2 EMERGENCY-RESET RECOVERY SELF-TESTS PASSED\n'
printf 'No host firewall, VPN connection, GUI session, or physical-path sentinel was changed by this self-test.\n'
printf 'Report: %s\n' "$REPORT"
