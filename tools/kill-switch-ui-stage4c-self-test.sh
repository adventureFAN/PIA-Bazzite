...
REPORT="$REPORT_DIR/pia-kill-switch-ui-stage4c-self-test.txt"
mkdir -p "$REPORT_DIR"
exec > >(tee "$REPORT") 2>&1

printf 'PIA Bazzite stage-4C optional runtime-status self-test\n'
...
if [[ -x "$ROOT/.venv/bin/python" ]]; then
  ...
  printf 'PASS  real MainWindow exercised all six optional runtime states offscreen\n\n'
else
...
printf '%s\n' '--- Optional runtime, UI wording, and state tests ---'
...
printf '\nALL STAGE-4C OPTIONAL RUNTIME-STATUS SELF-TESTS PASSED\n'
printf 'Report: %s\n' "$REPORT"
