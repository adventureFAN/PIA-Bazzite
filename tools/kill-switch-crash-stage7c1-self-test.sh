...
REPORT="$REPORT_DIR/pia-kill-switch-crash-stage7c1-self-test.txt"
mkdir -p "$REPORT_DIR"
exec > >(tee "$REPORT") 2>&1

printf 'PIA Bazzite stage-7C.1 retained-session takeover gate self-test\n'
...
printf '%s\n' '--- Stage-5/6 connection and Stage-7C.1 retained-session crash-takeover integration tests ---'
...
printf '\nALL STAGE-7C.1 RETAINED-SESSION CRASH-TAKEOVER GATE SELF-TESTS PASSED\n'
