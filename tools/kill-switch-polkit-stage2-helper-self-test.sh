...
REPORT="$REPORT_DIR/pia-kill-switch-polkit-stage2-helper-self-test.txt"
mkdir -p "$REPORT_DIR"
exec > >(tee "$REPORT") 2>&1

printf 'PIA Bazzite stage-2C installed-helper self-test\n'
...
for script in \
  ...
  "$ROOT/tools/kill-switch-polkit-stage2-helper-self-test.sh" \
  "$ROOT/tools/kill-switch-polkit-stage2-helper-namespace-test.sh"
do
  bash -n "$script"
done
printf 'PASS  stage-2C shell scripts parse\n\n'

printf '%s\n' '--- Restricted helper and Polkit unit tests ---'
...
printf 'ALL STAGE-2C UNPRIVILEGED SELF-TESTS PASSED\n'
printf 'Report: %s\n' "$REPORT"
