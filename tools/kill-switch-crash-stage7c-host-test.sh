...
REPORT="$REPORT_DIR/pia-kill-switch-crash-stage7c-host-test.txt"
RESET_UNIT="pia-bazzite-stage7c-takeover-safety-reset"
RESET_DELAY="15min"
TABLE="pia_bazzite_killswitch"
mkdir -p "$REPORT_DIR"
exec > >(tee "$REPORT") 2>&1

printf 'PIA Bazzite stage-7C real GUI crash takeover test\n'
printf 'Generated: %s\n' "$(date --iso-8601=seconds)"
printf 'This test verifies automatic clean-start status reconciliation, kills the\n'
printf 'protected GUI, restarts it, and requires exact adoption before a normal\n'
printf 'GUI disconnect is allowed to release the firewall.\n\n'
...
if sudo "$NFT_BIN" list table inet "$TABLE" >/dev/null 2>&1; then
  fail "A previous production firewall lock exists. Run the Stage-7C emergency reset first."
fi
...
if sudo systemctl is-active --quiet "${RESET_UNIT}.timer"; then
  ...
  fail "The independent Stage-7C safety-reset timer is not active."
fi
...
case "$DRIVER_STATUS" in
  0)
    ...
    printf '\nALL STAGE-7C REAL GUI CRASH-TAKEOVER HOST TESTS PASSED\n'
    printf 'Report: %s\n' "$REPORT"
    ;;
  20)
    cancel_reset_timer
    printf '\nStage 7C stopped before a production firewall lock was observed.\n'
    printf 'Report: %s\n' "$REPORT"
    exit "$DRIVER_STATUS"
    ;;
  *)
    ...
    printf '  cd %q && bash tools/kill-switch-crash-stage7c-emergency-reset.sh\n' "$ROOT"
    printf 'Report: %s\n' "$REPORT"
    exit "$DRIVER_STATUS"
    ;;
esac
