...
REPORT="$REPORT_DIR/pia-kill-switch-ui-stage4c1-self-test.txt"
mkdir -p "$REPORT_DIR"
exec > >(tee "$REPORT") 2>&1

printf 'PIA Bazzite stage-4C.1 runtime UI-polish self-test\n'
...
if [[ -x "$ROOT/.venv/bin/python" ]]; then
  ...
  printf 'PASS  polished MainWindow exercised all six runtime states offscreen\n\n'
else
...
bash -n \
  ...
  "$ROOT/tools/kill-switch-ui-stage4c-self-test.sh" \
  "$ROOT/tools/kill-switch-ui-stage4c1-self-test.sh"
printf 'PASS  stage-4 UI shell scripts parse\n\n'

printf '%s\n' '--- Optional runtime, wording, layout, and tooltip tests ---'
...
printf '\nALL STAGE-4C.1 RUNTIME UI-POLISH SELF-TESTS PASSED\n'
printf 'Report: %s\n' "$REPORT"
