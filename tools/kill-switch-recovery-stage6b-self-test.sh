...
REPORT="$REPORT_DIR/pia-kill-switch-recovery-stage6b-self-test.txt"
mkdir -p "$REPORT_DIR"
exec > >(tee "$REPORT") 2>&1

printf 'PIA Bazzite stage-6B.1 sentinel-race regression self-test\n'
printf 'Generated: %s\n' "$(date --iso-8601=seconds)"
printf 'This test does not use sudo, pkexec, networking, NetworkManager, nftables, or the leak sentinel.\n\n'
...
if [[ -x "$ROOT/.venv/bin/python" ]]; then
  ...
  printf 'SKIP  MainWindow runtime smoke test (.venv Python not found)\n\n'
fi
...
printf '%s\n' '--- Stage-5 connection and Stage-6 recovery tests ---'
...
printf '\nALL STAGE-6B.1 SENTINEL-RACE REGRESSION SELF-TESTS PASSED\n'
printf 'No host firewall, VPN connection, or physical-path sentinel was changed by this self-test.\n'
printf 'Report: %s\n' "$REPORT"
