...
REPORT="$REPORT_DIR/pia-kill-switch-ui-stage4b-self-test.txt"
mkdir -p "$REPORT_DIR"
exec > >(tee "$REPORT") 2>&1

printf 'PIA Bazzite stage-4B main-window UI self-test\n'
...
if [[ -x "$ROOT/.venv/bin/python" ]]; then
  ...
  printf 'PASS  real MainWindow preview constructed and exercised all four states offscreen\n\n'
else
...
printf '%s\n' '--- UI state-model, wording, and main-window safety tests ---'
...
printf '\nALL STAGE-4B MAIN-WINDOW UI SELF-TESTS PASSED\n'
printf 'Report: %s\n' "$REPORT"
