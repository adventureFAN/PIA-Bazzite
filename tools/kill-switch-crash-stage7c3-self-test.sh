...
REPORT="$REPORT_DIR/pia-kill-switch-crash-stage7c3-self-test.txt"
mkdir -p "$REPORT_DIR"
exec > >(tee "$REPORT") 2>&1

printf 'PIA Bazzite stage-7C.3 pipe-bound retained-session proof gate self-test\n'
...
printf '%s\n' '--- Stage-5/6 connection and Stage-7C.3 retained-session lifecycle and pipe-binding integration tests ---'
...
printf '\nALL STAGE-7C.3 PIPE-BOUND RETAINED-SESSION PROOF GATE SELF-TESTS PASSED\n'
